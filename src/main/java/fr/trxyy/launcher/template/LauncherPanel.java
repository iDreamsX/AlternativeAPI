...
public class LauncherPanel extends IScreen {
	...
	public LauncherPanel(Pane root, GameEngine engine) {
		...
		/** ===================== AFFICHER UN LOGO ===================== */
		this.drawLogo(engine, getResourceLocation().loadImage(engine, "alternative_logo.png"),
				engine.getWidth() / 2 - 165, 30, 330, 230, root, Mover.DONT_MOVE);
		...
		this.loginButton.setAction(ce -> {
			...
			if (this.usernameField.getText().length() < 3) {
				...
				GameAuth auth = new GameAuth(this.usernameField.getText(), this.passwordField.getText(),
						AccountType.OFFLINE);
				if (auth.isLogged()) {
	...
	private void update(GameEngine engine, GameAuth auth) {
		...
		/** ===================== REFAICHIR LE NOM DU FICHIER EN TELECHARGEMENT  ===================== **/
		this.timeline = new Timeline(new KeyFrame[] {
		new KeyFrame(javafx.util.Duration.seconds(0.0D), e -> timelineUpdate(engine),
